...
def inline_file_response(source, content_type, filename):
    ...
    `source` 支持本地路径与已打开的文件对象（存储适配 P-4：本地 / 对象存储统一入口）。
    P-4 拆分自 `file.py`（行数门禁），行为与下载口径同源（见 `record_base.py` 的 attachment 版本）。
    """
...
class FileAccessActionMixin:
    ...
    def download(self, request, *args, **kwargs):
        ...
        ``?direct=1``（P-4 预签名直连）：s3 后端且 boto3 可用时返回短时效预签名 URL
        （大文件不经服务端中转）；鉴权与审计先于签发完成，不可用时返回 ``direct=false``
        由调用方回退服务端中转下载（本地 / mirror 后端恒为回退，行为零变化）。
        """
        upload = self.get_object()
        # 存储适配（P-4）：本地 / 对象存储统一走 storage 原语，不再依赖本地绝对路径
    ...
    def config(self, request, *args, **kwargs):
        ...
        return ApiResponse(
            data={
                ...
                # F-8 上传策略下发（前端在选择文件后即时提示，避免上传后才被拒）
