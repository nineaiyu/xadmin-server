...
class TestBookCrudSmoke:
    ...
    def test_search_filter_by_name(self, auth_client, superuser, upload_file):
        Book.objects.create(
            name="Python入门", isbn="i1", author="a", admin=superuser, admin2=superuser, file=upload_file
        )
...
class TestBookDataPermissionIntegration:
    def test_non_owner_user_only_sees_own_books(
        self, api_client, normal_user, role, menu_factory, superuser, upload_file
    ):
