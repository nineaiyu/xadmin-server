...
| `common/base/magic.py` / `common/decorators/` / `common/core/config/` / `settings/models.py` | 后台任务/缓存/日志路径        | 保留           | 不直接进客户端响应                                                           |
...
| code    | 含义                             | 触发位置                            |
|---------|--------------------------------|---------------------------------|
| 1000    | 成功                             | ApiResponse 默认（`common/core/response.py`） |
...
| 1002    | 发送失败/参数处理失败（邮件、发码、上传类型校验等）     | verify_code、email 测试、`common/core/modelset/upload.py` |
| 1003    | 文件大小超限（上传）                     | `common/core/modelset/upload.py` |
