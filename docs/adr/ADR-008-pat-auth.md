...
- 状态：已接受
- 日期：2026-09-09
- 关联：N3 第二期 F3（执行计划已随 2026-09-12 文档清理删除，交付记录见 [metrics.md](../metrics.md) 2026-09-10 行）
...
15. **销项**：决策 11 的「近似口径」与「后果 · 演进」中登记的两项（创建时 IP 允许
    ...
    「凭证清理删除记录 → 审计断链」的问题。
16. **接口范围可勾选（2026-09-14 增补）**：scope 原本只能手填路径/正则，用户不知道
    写什么。新增 `GET personal-access-tokens/scope-options`：按权限菜单 × 本人角色
    （超管为全部启用的权限菜单，口径与请求鉴权同源）列出「我有权限的接口」，
    按父菜单分组、带方法与路径展示；勾选项写入的是**锚定正则**
    （`GET ^/api/system/user/?$`），只放行勾选的那一个接口，不连带放行同前缀的兄弟
    接口或相似前缀。历史/白名单/正则条目不落下：不在选项中的已存条目仍归到
    「自定义条目」可继续编辑；创建令牌时即可勾选（默认不勾 = 不限）。
    吊销改为二次确认（不可恢复操作）。
