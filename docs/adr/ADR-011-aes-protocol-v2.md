...
## 演进记录：v1 退役灰度开关（2026-09-12，下期规划 W4 实施项）

「旧格式解密路径保留至存量客户端自然消亡后再评估移除」的收敛措施落地：

- 新增配置 **`SECURITY_AES_V1_DECRYPT_ENABLED`**（默认 `True`，行为不变）：
  关闭后 `AESCipherV2.decrypt()` 对非 `v2:` 前缀密文一律返回空串（与非法输入同语义），
  v2 主路径不受牵连；
- 关闭前置条件：确认全量用户已升级至 v2 优先前端（前端非安全上下文仍会回退旧格式，
  关闭过早会使该场景登录失败——发布窗口核对前端版本分布后执行）；
- 配置缺失（Settings 未加载的极端场景）按开启处理，宁可多兼容不误杀；
- 备注：服务端 `AESCipherV2.encrypt()` 仍为旧格式实现，但生产代码无调用点
  （仅测试构造用例），v1 完全退役时可一并移除。
