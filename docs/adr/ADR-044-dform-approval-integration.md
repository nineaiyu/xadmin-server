...
- ~~控件继续扩展（金额 / 选人 / 级联）按需再做~~ → **已交付，见下节**。

## 增量（2026-09-18）：控件扩展三型（金额 / 选人 / 级联）

产品优先级确认后交付（长期优化方案 §4.4 动态表单行 / F4 按需功能池），控件集 11 → **14 种**。

- **`amount`（金额）**：数值控件加 `precision`（0–6，缺省不限）；提交侧按精度对拍
  （`abs(value - round(value, precision)) > 1e-9` 拒绝），min/max 复用数值口径；
- **`user`（选人）**：值 = 用户主键（`multiple=true` 时为 pk 数组，上限 20）；
  数据源统一 `GET .../user-options`（关键字搜索或按 pk 回显，≤20 条，仅 pk/用户名/昵称）——
  纯函数无 DB，**存在性不校验**（落库后由业务读取时兜底，登记边界）；权限与对应视图
  `list` 同口径（`common/core/permission.py` 特例，存量角色免重授权）；
- **`cascader`（级联）**：schema 的 options 为 `{value,label[,children]}` 树（≤3 层、
  节点总数 ≤200）；提交值是命中**叶子路径**的值序列（逐段比对，防伪造中间层取值）；
  设计器以「每行一条叶子路径、段以 `/` 分隔」的文本编辑（与明细子表列定义同款交互）；
- 前端填报/设计与报表 IM 接收人共用 `el-select` 远程搜索形态（昵称优先展示）；
- **测试**：`test_dform_widgets`（三型合法/非法/边界：精度、多选上限、路径非叶子、
  深度/节点数超限）、`test_dform_user_options`（搜索/回显/字段收敛）；E2E `dform.e2e.ts`
  扩展（金额精度、选人远程搜索、级联路径选择）。
