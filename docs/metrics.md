...
| 日期 | 阶段 | 变更摘要 |
|------|------|----------|
| 2026-09-26 | 全面优化重构计划收尾批（P1.1 观察项收口 / P2.8 盲区补测 / P3.5 双轨收敛，详录见仓库根 `OPTIMIZATION-PLAN.md`） | **P1.1 观察项清零**：修正 `check_cross_app_imports.py` 观察项正则 `\s` 跨行误报缺陷（空行后的模块级 import 被误报为函数级）；common 内 15 个文件的函数级业务 import 全部收口——`system/services.py` 新增 17 个薄委托函数 + `UserRole` / `API_ACTION_SPECS` 惰性导出，`settings/services.py` 导出 `Setting`，gate/credentials 用「模块命名空间 + 调用期属性访问」保留迁移期降级；`CONTRACT_SEAMS` 台账 11 → **28** 条。**P2.8 盲区补测**：`.coveragerc` 把 `settings` / `mfa` app 纳入覆盖率测量（`*/settings/*` omit 收窄为 `server/settings/*`）；新增 68 例（mfa 后端 sms/email/otp/passkey/注册表/confirm 单测、settings 邮件/短信连通性诊断 API 集成测试、`dump_init_json` 导出半环、Setting 模型与密码规则工具）；`dump_init_json` 此前零测试覆盖。**P2.8 前端阈值二步**：502 → **512** 测试（webauthn / message 单测 + RePermissionPreview hook 挂载失败 unhandled rejection 根修），实测 53.09/55.42/42.01/52.96，阈值 51/50/39/51 → **52/54/41/52**。**P3.5 双轨收敛**：主 `tsconfig.json` 翻 `strict: true`（原 noImplicitAny ~350 处存量已清零），e2e 并入主配置 include，删除 `tsconfig.strict.json` + `typecheck-strict.mjs` + `typecheck:strict` 脚本与 CI 步骤——strict 轨道只跑 vue-tsc 的盲区暴露 6 处 tsc implicit any（.tsx 内 SFC emit 回调），以共享 `CropperPayload` 类型 + 调用点注解清零。**门禁修复**：巨型文件门禁清掉两处未登记超限（P2.7 遗留）——`common/core/fields.py` 567 → 481（字典字段拆分 `fields_dict.py` 兼容 re-export）、`server/settings/base.py` 506 → 493（HTTPS 安全头拆分 `security_https.py`）；`RePermissionPreview` hook 挂载失败改为空态吞错（显式 load 仍上抛） | 后端 pytest **3575 passed**（+68）/ 2 skipped / 覆盖率 **88.12%**（门禁 85）+ ruff / 行数 / 跨 app / Django `check --database` / 文档五件套；前端 vitest **512 passed**（77 文件）、typecheck（strict 单轨）、eslint / prettier、i18n 对称、契约、版本、行数、包体 **546 KB**（+8.6 KB / 预算 15 KB）；`check_cross_app_imports` 观察项 **0**（契约缝 28 条登记） |
| 2026-09-25 | 体验升级与工程优化计划（UI 视觉 / 框架性能 / 二开）+ 跑批确定性治理（详录见 [plans/体验升级与工程优化计划-2026.09.md](plans/体验升级与工程优化计划-2026.09.md)） | **UI 视觉（V1~V8）**：视觉基线 6 → 7 页（按「结构稳定、数据稳定」口径排除监控 / AI 控制台 / 操作日志）；应用外观层 `appearance.scss`（卡片圆角/阴影/hover 体系）；`ReEmpty` / `ReSkeleton` + 全站 30 处空态收敛；大屏视觉重构（玻璃卡 + 图标控制键 + 画框图表）；图表观感（`chartTheme.withAlpha` + 圆角柱/面积渐变）；面板层次与上传/更多图标语义收敛。**框架与性能（P1~P6）**：P1 EP 体积实测后回滚（9 组件下沉 + 去 vendor 分组仅 −1.2 KB）、P2 `_with_meta` 实测判定「内联本就是净收益」不优化、P3 perf 基线 3 → **5 页**、P4 移除未使用依赖 `vue-virtual-scroller`、P5 路由级空闲预取（+9 例单测）、P6 评估收口（登录 LCP ≈1.2s 属 dev 链路基线；i18n zh 必须 eager，en 已懒加载）——P1/P2 结论与 P6 触发条件均已登记。**二开（D1~D3）**：`asset` 示例全链路走查（建模型 → `generate_crud` → 门禁 → 真实 API 链路），修复生成器两处缺陷（新 app import 分组 I001、缺默认排序）并加 6 例守护。**跑批确定性（T1~T7）**：a11y 空树 empty-block 采样（探针定性）、审批角标跨 spec 数据耦合、存储探针固定名并发踩踏（旧实现 240 次并发失败 206 次 → 0）、陈旧「更多」定位、暗色皮肤共享态、Office soffice profile 冲突、遗留探针清理 | pytest **3476 collected / 全绿**（exit=0）+ ruff / 行数 / 跨 app / 缓存键 / makemigrations / 文档五件套；前端 vitest **498**、typecheck+strict、eslint / prettier / stylelint、i18n、契约、行数、包体 **547.5 KB**（预算 15 KB 内）；E2E：全量并行 4 分片跑批 + 点验 `a11y`+`approval` 双浏览器 **12 passed**、`theme-dark` / `relation-invite-export` / `office-preview` 双浏览器复跑通过 |
| 2026-09-23 | 平台能力交付与收口（安全与决策信息 / AI 平台化 / 治理与好用功能 / 交付工程） | **能力交付**：安全与决策信息（AI 引用数据护栏=引用块包裹+注入告警+输出与流式脱敏、凭据统一注册与轮换命令+只读凭据页、审批决策快照=变更前后对比+业务对象引用+处理人显示名快照、资源影响面前置预检 8 类、表单错误跨页签定位与随机密码生成）；AI 平台化（双轨调用=原生 function calling / 提示词 JSON + 能力探测 + 用量配额 + 通用标签 + 任务中心=协作取消/白名单重跑/统一进度助手）；治理与好用（批量更新、通知模板沙箱渲染、我的视图个人取值域、审批协作=讨论区+抄送、账号巡检、登录策略=优先级命中+预演+并发会话上限、文件访问审计+上传扩展名策略、Passkey/WebAuthn、OIDC、可插拔存储 local/s3+迁移命令、审计冷归档=水位驱动清理、关联计数、受控 lookup 与 `?fields=`、邀请开户与账号有效期、表头排序、高级筛选、图表/大屏图片导出）。**交付工程**：依赖单源 `pyproject.toml` + `uv.lock`（requirements 为导出产物）、system 迁移 0005~0013 压缩为单个合并迁移、代码与文档去台账编号 519 处。**收口实测**：pytest 全量 **3426 collected / 0 失败 / 覆盖率 87.44%**（门禁 85）+ ruff/行数/跨 app/缓存键/makemigrations；前端 vitest **347**、typecheck/strict 全仓、eslint/prettier/stylelint、契约、i18n 2654↔2654、包体 537.4 KB（预算内）、行数 0 超标；E2E 收口修复**动态路由组件解析「子串取首个命中」劫持页面**的框架缺陷（消息设置页曾整页渲染成消息模板子组件且无报错；新增纯函数解析器 + 7 例守护单测）+ 确认层定位收敛到可见态 + 审批状态断言改轮询（落库竞态）；回归 system-pages/smoke/a11y/config-persist/locale/notify-im 双浏览器 **52 passed**，审批链/转交/导入导出系列复跑全绿。本地容器逐页验收顺带修复：验证码配置缺 category 500、非模型视图集元数据 500、标签过滤 400 |
