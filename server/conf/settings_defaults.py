...
SETTINGS_CONFIG = {
    ...
    # 文件存储后端（声明式可插拔）：local（默认，本地磁盘）/ s3（对象存储；
    ...
    # 登录日志保留天数（冷归档自动面：归档水位驱动清理；0 = 不自动清理）
    ...
    # 账号到期提醒：到期前 N 天发站内信 + 邮件（每日任务）；0 = 关闭提醒
